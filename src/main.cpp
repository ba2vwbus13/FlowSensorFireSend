...
void readCurveSensor(float *curve) {
  float value = analogRead(CH_PIN);
  float dV = value * MAX_VOLTAGE / ANALOG_MAX;
  float dR = -1.0;
  if( 0.005 < (5.0 - dV) )
  {
...
void loop() {
    ...
    img.fillSprite(0);
    sprintf(text_string, "Sen(%d)", flow_sensor_number);
    img.drawString(text_string, 10, 20, 4);  // draw string with padding.  绘制带有填充的字符串
    sprintf(text_string, "MAG X: %.2f(%.2f)", dev.data.x, dev.data.x - mag_offset.x);
    img.drawString(text_string, 10, 50, 4);  // draw string with padding.  绘制带有填充的字符串
    sprintf(text_string, "MAG Y: %.2f(%.2f)", dev.data.y, dev.data.y - mag_offset.y);
    img.drawString(text_string, 10, 80, 4);
    sprintf(text_string, "MAG Z: %.2f(%.2f)", dev.data.z, dev.data.z - mag_offset.z);
    img.drawString(text_string, 10, 110, 4);
    sprintf(text_string, "HEAD Angle: %.2f", angle);
    img.drawString(text_string, 10, 140, 4);
    sprintf(text_string, "Curve: %.2f", curve);
    img.drawString(text_string, 10, 170, 4);
    img.drawCentreString("Press BtnA enter calibrate", 160, 210, 4);
    img.pushSprite(0, 0);

    oldangle = angle;

    // 画面表示
    // テキストフォント：26ピクセルASCIIフォント
    sprintf(text_string, "fs:%d mx:%.2f my:%.2f sp:%.2f", 
        flow_sensor_number, 
        dev.data.x - mag_offset.x, 
        dev.data.y - mag_offset.y, 
        curve);
    LoRaCommand(text_string);
    delay(300);
